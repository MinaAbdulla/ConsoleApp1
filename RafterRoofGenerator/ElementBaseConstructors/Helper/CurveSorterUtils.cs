namespace RafterRoofGenerator.ElementBaseConstructors.Helper
{
    #region Header
    //
    ...
    //
    // Modified and addition of some functions on different
    // Occasions up to 6/4/2018.
    //
    ...
    return e;
          ...
          bool acceptDerivedClass)
        {
            ...
            {
                e = CurveSorterUtils.SelectSingleElement(
                  uidoc, description);
            }
      ...
      return elementsByName.First<Element>();
        ...
        #region OperationsOnPolygonsAndCurves

        /// <summary>
        /// Finds out if the 2 curves in a polygon are equal due to their
        /// start and end points.
        /// </summary>
        /// <param name="c1">First curve</param>
        /// <param name="c2">Second curves</param>
        /// <returns>Bool Is Curve Equal the other</returns>
        public static bool IsCurveEqual(Curve c1, Curve c2)
        {
            var startPoint1 = c1.GetEndPoint(0);
            var startPoint2 = c2.GetEndPoint(0);
            var endPoint1 = c1.GetEndPoint(1);
            var endPoint2 = c2.GetEndPoint(1);
        ...
        /// <returns></returns>
        public static List<XYZ> ExtractPolygonVertices(CurveArray curves)
        {
        ...
        /// <param name="curves"></param>
        /// <returns></returns>
        public static List<XYZ> ExtractPolygonVertices(List<Curve> curves)
        {
            var pointList = new List<XYZ>();
            foreach (Curve c in curves)
            {
        ...
        /// <summary> Calculate area of polygon outline.
        /// For clockwise are will be less then zero. </summary>
        /// <param name="polygon">List of polygon points.</param>
        /// <returns> Area. </returns>
        private static double Area(CurveArray curves)
        {
            var polygon = ExtractPolygonVertices(curves);
            var n = polygon.Count;
            double A = 0.0f;
        ...
        /// <param name="polygon">List of polygon points.</param>
        /// <returns> Area. </returns>
        private static double Area(List<Curve> curves)
        {
            var polygon = ExtractPolygonVertices(curves);
            var n = polygon.Count;
            double A = 0.0f;
        ...
        public static bool IsClockwisePolygon(CurveArray curves)
        {
            return Area(curves) < 0;
        }

        /// <summary> Check if polygon is clockwise. </summary>
        /// <param name="polygon"> List of polygon points. </param>
        /// <returns> If polygon is clockwise. </returns>
        public static bool IsClockwisePolygon(List<Curve> curves)
        {
            return Area(curves) < 0;
        }
        ...
        /// <summary>
        /// Makes a polygon counter clock wise.
        /// </summary>
        /// <param name="polygon"></param>
        /// <returns></returns>
        public static CurveArray MakeCounterClockwise(CurveArray polygon)
        {
        ...
        /// <param name="polygon"></param>
        /// <returns></returns>
        public static List<Curve> MakeCounterClockwise(List<Curve> polygon)
        {
            if (IsClockwisePolygon(polygon))
                polygon.Reverse();
            return polygon;
        ...
        /// <param name="polygon"></param>
        /// <returns></returns>
        public static CurveArray MakeClockwise(CurveArray polygon)
        {
            if (IsClockwisePolygon(polygon))
                return polygon;
            else
                return polygon = ReversePolygon(polygon);
        }
        ...
        /// <param name="polygon"></param>
        /// <returns></returns>
        public static List<Curve> MakeClockwise(List<Curve> polygon)
        {
            if (IsClockwisePolygon(polygon))
            {
                return polygon;
        ...
        /// <summary>
        /// Reverses a given CurveArray of polygons.
        /// </summary>
        /// <param name="polygon">Given Polygon.</param>
        /// <returns>Reversed Polygon.</returns>
        public static CurveArray ReversePolygon(CurveArray polygon)
        {
            CurveArray reversedPolygon = new CurveArray();
            List<Curve> newPolygon = new List<Curve>();
            ...
            newPolygon.Reverse();
            foreach (var c in newPolygon)
            {
                reversedPolygon.Append(c);
            }
    ...
    public static class JtLineExtensionMethods
    {
        ...
        public static bool Contains(
          this Line line,
          XYZ p,
          double tolerance = CurveSorterUtils._eps)
        {
    ...
    public static class JtPlaneExtensionMethods
    {
        ...
        {
            Debug.Assert(
              CurveSorterUtils.IsEqual(plane.Normal.GetLength(), 1),
          ...
          XYZ p)
        {
            ...
            Debug.Assert(
              CurveSorterUtils.IsZero(plane.SignedDistanceTo(q)),
